...
class Chart {
    ...
    generalisedLineChart(options) {
        ...
        x.domain(d3.extent(data, (d) => d[options.xField]));
        y.domain([
            0,
            Math.round((1.1 *
                d3.max(data, function (d) {
                    return d[options.yField];
                })) /
                options.rounding) * options.rounding,
        ]);
        ...
        types.forEach((type) => {
            const typeData = chart.data.filter((d) => d[options.filter] === type.label);
            chart.plot
                .append('path')
                .datum(typeData)
                .attr('class', 'line')
    ...
    lineChart() {
        ...
        console.log('Date extent...', d3.extent(data, function (d) {
            return d.date;
        }));
        ...
        const months = Object.keys(monthSet).map(function (month) {
            return {
    ...
    circle() {
        ...
        table
            ...
            row
                ...
                // console.log("hello!");
                d3.select(`#circle-${camelize(d.name)}`).attr('fill', d3.schemeCategory10[i % 10]);
            })
                .on('mouseout', function () {
                // console.log("hello!");
                d3.select(`#circle-${camelize(d.name)}`).attr('fill', 'rgba(0,0,0,0.05)');
            });
            row
                .append('rect')
                .attr('width', 200)
    ...
    squares() {
        ...
        table
            ...
            row
                .on('mouseover', function () {
                // console.log("hello!");
                d3.select(`#square-${camelize(d.name)}`).attr('fill', d3.schemeCategory10[i % 10]);
            })
                .on('mouseout', function () {
                // console.log("hello!");
                d3.select(`#square-${camelize(d.name)}`).attr('fill', 'rgba(0,0,0,0.05)');
            });
            row
                .append('rect')
                .attr('width', 200)
    ...
    drawMap(options) {
        ...
        Promise.all([d3.json(json), d3.json(usa), d3.json(aus)]).then(([json, usa, aus]) => {
    ...
    venn(options) {
        ...
        const mid = svg
            ...
            .classed('mid', true)
            .classed('activeShape', true);
        mid.append('path').attr('d', `M ${arcx} ${arcy1}
        ...
        $('.activeShape')
            ...
            if (el.hasClass('left'))
                el.on('click', options.leftAction);
            if (el.hasClass('mid'))
                el.on('click', options.midAction);
            if (el.hasClass('right'))
                el.on('click', options.rightAction);
        });
        svg
            .append('text')
            .text(options.left)
...
function decorateTable(dataset, newOptions) {
    const element = newOptions ? newOptions.element : '#dataset table';
    const columns = dataset.columns
        ? dataset.columns.map(function (d) {
            return {
                title: d,
                data: d,
            };
        })
        : Object.keys(dataset[0]).map(function (d) {
            return {
                title: d,
                data: d,
            };
